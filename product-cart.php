<html lang="en" style="height: auto;"><head>
  ...
  <?php 
    ...
    if(isset($_POST['delete_product'])){
        ...
        $sql = "Delete from cart_item where book_number = '$product_id'";
        $result = mysqli_query($con, $sql);
        if($result){
    ...
    if(isset($_POST['add_amount'])){
        ...
        $sql = "UPDATE cart_item SET amount = amount + 1 WHERE book_number = $product_id";
        $result = mysqli_query($con, $sql);
        if($result){
            $product_id = $_POST['add_amount'];
    ...
    if(isset($_POST['sub_amount'])){
     ...
     if(isset($_POST['add_to_cart'])){
         ...
         if(isset($_SESSION['mycart'][$book_number])){
            //Increase amount
            $sql = "UPDATE cart_item SET amount = amount + 1 WHERE book_number = $book_number";
            $result = mysqli_query($con, $sql);
            if ($result) {
             ...
             //insert cart item in db
            $cart_id = $_SESSION['cart_id'];
            echo $cart_id ;

            $sql = "Insert into cart_item (book_number, amount, price, cart_id) values ('$book_number', 1, (select price from books where book_number = '$book_number'), '$cart_id')";
            $result = mysqli_query($con, $sql);
            if($result){
